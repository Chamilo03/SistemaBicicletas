...
public class UsuarioController {
	...
	@Operation(summary = "Listar todos los usuarios", description = "Obtiene una lista completa de los usuarios registrados en el sistema.")
	...
	@Operation(summary = "Login de usuario", description = "Permite iniciar sesión con email y contraseña válidos.")
	...
	@Operation(summary = "Recargar saldo de un usuario", description = "Permite aumentar el saldo de un usuario mediante una recarga externa (por ejemplo, desde Stripe).")
	...
	@Operation(summary = "Verificar existencia de un usuario", description = "Verifica si un usuario con el ID especificado existe en el sistema. Utilizado por otros servicios (por ejemplo, el microservicio de pagos).")
	@ApiResponse(responseCode = "200", description = "Usuario válido")
	@ApiResponse(responseCode = "404", description = "Usuario no encontrado", content = @Content)
	@GetMapping("existe/{id}")
	public ResponseEntity<String> existeUsuario(@PathVariable Long id) {
		if (!service.findById(id)) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Usuario no encontrado");
		}
