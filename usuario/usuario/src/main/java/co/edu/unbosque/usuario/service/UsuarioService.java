...
public class UsuarioService {
	...
	public String iniciarPago(Long id, double monto) {
		try {
			RestTemplate restTemplate = new RestTemplate();
			String urlPagos = "http://localhost:8083/api/pagos/pago?id=" + id + "&amount=" + (long) monto;
			String urlPagoStripe = restTemplate.postForObject(urlPagos, null, String.class);
			return urlPagoStripe;
		} catch (Exception e) {
			throw new RuntimeException("Error comunicándose con el servicio de pagos: " + e.getMessage());
		}
	}
